...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(await instance.name.call(), 'Night Lights');
    assert.equal(await instance.symbol.call(), 'NTLT');
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    // 1. create a Star with different tokenId
    await instance.createStar('Unique Star Name', starId, {from: user1});

    // 2. Call your method lookUptokenIdToStarInfo
    let starName = await instance.lookUptokenIdToStarInfo(starId);

    // 3. Verify if you Star name is the same
    assert.equal(starName, 'Unique Star Name');
});
